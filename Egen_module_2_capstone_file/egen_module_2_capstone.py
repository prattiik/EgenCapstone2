...
DAG_NAME = "egen_module_2_capstone_dag"
...
def get_ensemble_records():
    ...
    bucket = storage_client.get_bucket('egen_crypto_data')
...
def clean_and_process_records():
    ...
    # Filtering the records that belong to only bitcoin
    read_ensembled_df = read_ensembled_df[read_ensembled_df["id"] == 'BTC']
    read_ensembled_df.sort_values("price_timestamp", inplace=True)
    new_ensembled_df = read_ensembled_df[['id', 'currency', 'symbol', 'name', 'price', 'price_timestamp']].copy()
    new_ensembled_df.columns = ['id', 'currency', 'symbol', 'name', 'price', 'price_timestamp']

    new_ensembled_df.to_csv('/home/airflow/gcs/data/capstone/clean_records.csv', index=False)


def upload_to_bigquery():
    client = bigquery.Client()

    table_id = 'egendemo.crypto_price.bitcoin_price'
